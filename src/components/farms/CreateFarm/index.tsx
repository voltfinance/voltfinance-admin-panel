...
interface IFormState {
    ...
    rewardRateMin: string | undefined;
    rewardRateMax: string | undefined;
    bonusRewardRate: string | undefined;
    bonusRewardRateMin: string | undefined;
    bonusRewardRateMax: string | undefined;
}
...
const CreateFarm = () => {
    ...
    const [isLimitRates, setIsLimitRates] = useState(false);
    ...
    const handleLimitRatesToggle = () => {
        setIsLimitRates(!isLimitRates);
        setValue("rewardRateMin", undefined);
        setValue("rewardRateMax", undefined);
        setValue("bonusRewardRateMin", undefined);
        setValue("bonusRewardRateMax", undefined);
    };
    ...
    const rewardRateMin = watch("rewardRateMin");
    const rewardRateMax = watch("rewardRateMax");
    const bonusRewardAmount = watch("bonusRewardAmount");
    const bonusRewardRate = watch("bonusRewardRate");
    const bonusRewardRateMin = watch("bonusRewardRateMin");
    const bonusRewardRateMax = watch("bonusRewardRateMax");
    ...
    const rewards: IRewards = {
        ...
        rewardRateMinBn: rewardRateMin && rewardToken ? parseUnits(rewardRateMin as `${number}`, rewardToken.decimals) : undefined,
        rewardRateMaxBn: rewardRateMax && rewardToken ? parseUnits(rewardRateMax as `${number}`, rewardToken.decimals) : undefined,
        ...
        bonusRewardRateMinBn:
            bonusRewardRateMin && bonusRewardToken ? parseUnits(bonusRewardRateMin as `${number}`, bonusRewardToken.decimals) : undefined,
        bonusRewardRateMaxBn:
            bonusRewardRateMax && bonusRewardToken ? parseUnits(bonusRewardRateMax as `${number}`, bonusRewardToken.decimals) : undefined,
    };
                        ...
                        <div className="flex flex-col gap-4">
                            <div className="flex flex-col md:flex-row justify-between">
                                ...
                                <div className="flex items-center gap-2 text-sm">
                                    <label htmlFor="rateLimits">Limit Rates</label>
                                    <Switch id="rateLimits" checked={isLimitRates} onCheckedChange={handleLimitRatesToggle} />
                                </div>
                                <div className="flex items-center gap-2 text-sm">
                        ...
                        {isLimitRates && (
                            <div className="w-full flex justify-between items-center gap-4">
                                <RewardRate
                                    control={control}
                                    name={"rewardRateMin"}
                                    title={`Enter min. ${rewardToken?.symbol || "Reward"} Rate limit`}
                                    isRequired
                                    maxDecimals={rewardToken?.decimals}
                                    disabled={!rewardToken || !rewardAmount}
                                />
                                <RewardRate
                                    control={control}
                                    name={"rewardRateMax"}
                                    title={`Enter max. ${rewardToken?.symbol || "Reward"} Rate limit`}
                                    isRequired
                                    maxDecimals={rewardToken?.decimals}
                                    disabled={!rewardToken || !rewardAmount}
                                />
                            ...
                            <>
                                ...
                                {isLimitRates && (
                                    <div className="w-full flex justify-between items-center gap-4">
                                        <RewardRate
                                            control={control}
                                            name={"bonusRewardRateMin"}
                                            title={`Enter min. ${bonusRewardToken?.symbol || "Bonus Reward"} Rate limit`}
                                            isRequired
                                            maxDecimals={bonusRewardToken?.decimals}
                                            disabled={!bonusRewardToken || !bonusRewardAmount}
                                        />
                                        <RewardRate
                                            control={control}
                                            name={"bonusRewardRateMax"}
                                            title={`Enter max. ${bonusRewardToken?.symbol || "Bonus Reward"} Rate limit`}
                                            isRequired
                                            maxDecimals={bonusRewardToken?.decimals}
                                            disabled={!bonusRewardToken || !bonusRewardAmount}
                                        />
