...
const InputNumber = ({ title, name, control, disabled, isRequired, maxDecimals = 18 }: { title: string, name: keyof IFormState, control: Control<IFormState>, disabled?: boolean, isRequired?: boolean, maxDecimals?: number }) => ( 
    <Controller 
        name={name}
        disabled={disabled}
        control={control}
        ...
        render={({ field: { onChange, value }, fieldState: { invalid } }) => <div className={cn('flex items-end w-full h-[62px] relative group bg-white rounded-2xl border border-solid border-[#eaeaea] focus-within:bg-white focus-within:border-[#8248E5]', disabled ? 'opacity-30 pointer-events-none' : '')}>
            <input 
                id={name} 
                required={isRequired} 
                autoComplete="off" 
            ...
            <label htmlFor={name} className={
                isRequired ?
                    ...
                    :
                    `transform transition-all absolute top-0 left-0 h-full flex items-center pl-5 text-[16px] cursor-text group-focus-within:text-xs group-focus-within:h-1/2 group-focus-within:-translate-y-[0px] group-focus-within:translate-x-[16px] group-focus-within:opacity-40 group-focus-within:pl-0`}>{title}</label>
            {!value && <span className="absolute text-red-500 top-4 right-5 text-[22px]">*</span>}
            {!invalid && !disabled && <span className={`absolute text-[#DB0170] top-5 right-5 text-[22px] peer-valid:inline peer-invalid:hidden`}>
...
const RewardRate =({ title, name, control, isRequired, maxDecimals, disabled }: { title: string, name: keyof IFormState, control: Control<IFormState>, isRequired?: boolean, maxDecimals?: number, disabled?: boolean }) => <div>
    ...
    return <Controller 
        ...
        render={({ field: { onChange, value } }) => (<>
            <div className="flex w-full gap-4">
                <TokenSelectorModal isOpen={isOpen} setIsOpen={setIsOpen} onSelect={(currency) => {
                    setIsOpen(false)
                    onChange(currency)
                    setCurrency(currency)
                }}>
                    <button className={`flex items-center justify-between gap-2 w-full px-4 py-3 border rounded-2xl text-left hover:bg-gray-50`} onClick={() => setIsOpen(true)}>
                        <span className="inline-flex items-center gap-2">
                            { value && <CurrencyLogo currency={value} size={20} /> }
                            <span>{value ? value.symbol : `Select Token`}</span>
                            { value && balance && <span className="px-2 bg-gray-100 rounded-xl">{`${formatCurrency.format(+balance.formatted)}`}</span> }
                        </span>
...
const CreateFarm = () => {
    ...
    return <div className="grid grid-cols-1 md:grid-cols-2 md:gap-8 w-full">
        ...
        <div className="flex flex-col gap-4 p-8 -mx-8 md:mx-0 h-fit md:border md:rounded-xl">
            ...
            { pool ? isFarmingLoading ? <Loader size={18} color="currentColor" /> : isPoolAvailable ? <span className="text-green-500">Pool is available</span> : <span className="text-red-500">This pool already has active farming</span> : null }
        </div>

        { isPoolAvailable && <div className="p-8 -mx-8 md:mx-0 md:border rounded-xl">
            ...
            <form className="flex flex-col gap-4 " onSubmit={handleSubmit(onSubmit)}>
                ...
                <InputNumber control={control} name={'rewardAmount'} title={`Enter ${rewardToken?.symbol || 'Reward'} Amount`} isRequired maxDecimals={rewardToken?.decimals} disabled={!rewardToken} />

                <RewardRate control={control} name={'rewardRate'} title={`Enter ${rewardToken?.symbol || 'Reward'} Distirbution Rate per second`} isRequired maxDecimals={rewardToken?.decimals} disabled={!rewardToken || !rewardAmount} />
...
const PoolSelector = ({ control, reset }: { control: Control<IFormState>, reset: () => void }) => {
    ...
    return <Controller
        ...
        render={({ field: { onChange } }) => <Select onValueChange={v => {
            reset()
            ...
            <SelectContent>
                { formattedPools.map(pool => <SelectItem key={pool.id} value={pool.id} className="px-2">
