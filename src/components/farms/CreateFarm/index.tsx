...
const CreateFarm = () => {
    ...
    const rewards: IRewards = {
        reward: rewardAmount,
        rewardBn:
            rewardAmount && rewardToken ? parseUnits(rewardAmount as `${number}`, rewardToken.decimals) : undefined,
        rewardRate: rewardRate,
        rewardRateBn:
            rewardRate && rewardToken ? parseUnits(rewardRate as `${number}`, rewardToken.decimals) : undefined,
        bonusReward: bonusRewardAmount,
        bonusRewardBn:
            bonusRewardAmount && bonusRewardToken
                ? parseUnits(bonusRewardAmount as `${number}`, bonusRewardToken.decimals)
                : undefined,
                        ...
                        {formattedPools.map((pool) => (
                            <SelectItem key={pool.id} value={pool.id} className="px-2">
                                <div className="flex w-full gap-1">
                                    <CurrencyLogo
                                        currency={new Token(DEFAULT_CHAIN_ID, pool.token0.id, +pool.token0.decimals)}
                                        size={20}
                                    />
                                    <CurrencyLogo
                                        currency={new Token(DEFAULT_CHAIN_ID, pool.token1.id, +pool.token1.decimals)}
                                        size={20}
                                    />
