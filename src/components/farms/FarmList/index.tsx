...
const FarmRow = (farm: FarmingFieldsFragment) => {
    ...
    return (
        <div className="grid grid-cols-1 gap-4 md:gap-0 md:grid-cols-5 w-full text-left p-4 bg-gray-50 border border-gray-300 rounded-xl">
            {token0 && token1 ? (
...
const FarmList = () => {
    ...
    const { activeFarms, deactivatedFarms } = useMemo(() => {
        ...
        return farms.eternalFarmings.reduce<{
            activeFarms: FarmingFieldsFragment[];
            deactivatedFarms: FarmingFieldsFragment[];
        }>(
            (acc, farm) => {
