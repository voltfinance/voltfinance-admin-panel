...
import { usePrepareEternalFarmingSetWeights, useAlgebraVirtualPoolFeeWeights } from "@/generated";
...
const ManageWeightsModal = ({ title, incentiveKey, virtualPool, children }: IManageWeightsModal) => {
    const { data: initialWeights } = useAlgebraVirtualPoolFeeWeights({
        address: virtualPool,
    });
    ...
    useEffect(() => {
        if (initialWeights) {
            setWeight0(initialWeights[0] / 10);
            setWeight1(initialWeights[1] / 10);
        }
    }, [initialWeights]);
