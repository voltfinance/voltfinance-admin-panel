...
const FarmRewardDetails = ({ token, rate, reward, incentiveKey, isBonus, rewardRates, isDeactivated }: IFarmRewardDetails) => {
    ...
    const rewardRate = useMemo(() => {
        ...
        switch (rewardRateSpan) {
            case RewardRateSpan.SECOND: return `${ratePerSecond} ${token.symbol} per sec`
            case RewardRateSpan.DAY: return `${ratePerDay} ${token.symbol} per day`
            case RewardRateSpan.MONTH: return `${ratePerMonth} ${token.symbol} per month`
            default: return `${ratePerSecond} ${token.symbol} per sec`
        }
    ...
    const rewardLeftFor = useMemo(() => {
        ...
        switch (rewardLeftForSpan) {
            case RewardLeftForSpan.MINUTES: return `${leftForMinutes} min`
            case RewardLeftForSpan.HOURS: return `${leftForHours} hr`
            case RewardLeftForSpan.DAYS: return `${leftForDays} days`
        }
    ...
    return <div className="text-left p-4 border rounded-xl">
        ...
        <div>
            ...
            <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                    <button onClick={() => setRewardLeftForSpan(RewardLeftForSpan.MINUTES)} className={cn('py-1 px-2 rounded-md', rewardLeftForSpan === RewardLeftForSpan.MINUTES ? 'bg-blue-500 text-white font-bold border' : 'border')}>Min</button>
                    <button onClick={() => setRewardLeftForSpan(RewardLeftForSpan.HOURS)} className={cn('py-1 px-2 rounded-md', rewardLeftForSpan === RewardLeftForSpan.HOURS ? 'bg-blue-500 text-white font-bold border' : 'border')}>Hour</button>
                    <button onClick={() => setRewardLeftForSpan(RewardLeftForSpan.DAYS)} className={cn('py-1 px-2 rounded-md', rewardLeftForSpan === RewardLeftForSpan.DAYS ? 'bg-blue-500 text-white font-bold border' : 'border')}>Day</button>
                </div>
            </div>
        </div>
        </> }
